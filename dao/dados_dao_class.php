...
class DadosDAO extends PDOConnectionFactory {
	...
	public function findTotaisCirByUfId($ufId=0) {
		...
		$sql .=" count(*) as 'numcir',";
		$sql .=" ((count(*)/436)*100) as 'perc_tot_cir',";
		$sql .=" sum(n_muni_cir) as 'num_munics',";
		$sql .=" ((sum(n_muni_cir)/5565)*100) as perc_tot_munics,";
		$sql .=" sum(pop_total_2013) as pop2013,";
		$sql .=" ((sum(pop_total_2013)/192379287)*100) as perc_tot_pop,";
		$sql .=" avg(n_muni_cir) as 'media_mun_cir',";
		$sql .=" sum(pop_total_2013)/sum(n_muni_cir) as 'media_pop_mun',";
		$sql .=" (sum(benef13)/(sum(pop_total_2013))*100) as 'perc_benefi',";
		//$sql .=" (sum(esf_pop_11)/(sum(pop_total_2011))*100) as 'perc_pop_cadast_esf',";
		$sql .=" (sum(med_tot_dez13)/sum(pop_total_2013))*1000 as 'perc_med_mil_hab',";
		$sql .=" (sum(med_sus_dez13)/sum(med_tot_dez13))*100 as 'perc_med_sus',";
		$sql .=" (sum(leit_tot_dez13)/sum(pop_total_2013))*1000 as 'perc_leito_mil_hab',";
		$sql .=" (sum(leit_sus_dez13)/sum(leit_tot_dez13))*100 as 'perc_leitos_sus'";
		...
		$stmt->execute();
                ...
                $arrIndics= array();
    	        foreach ($stmt as $row){
    		   $arrIndics[$row[0]] = array($row[1],$row[2],$row[3],$row[4],$row[5],$row[6],$row[7],$row[8],$row[9],$row[10],$row[11],$row[12],$row[13]);
		}
	 ...
	 * 
	 */
	public function findTotaisCirByUfIdAndGrupo($ufId=0,$grupo="grupo_socio") {
                        ...
                        foreach($resConsulta as $indice => $tabelas){
                            ...
                            foreach($tabelas as $campo => $valor){
                                ...
                                if (is_numeric($valor) && stripos($valor, ".")) {
                                    ...
                                    if(!in_array($campo, $arrAlinhamento)){
                                        $tabela .= ($valor?(is_numeric($valor)?number_format($valor, 0, ',', '.'):$valor):(!is_numeric($valor)?'-':$valor));
                                    }else{
                        ...
                        foreach($resConsulta as $indice => $tabelas){
                            foreach($tabelas as $campo => $valor){
                                if (is_numeric($valor) && stripos($valor, ".")) {
                                    ...
                                    if(!in_array($campo, $arrAlinhamento)){
                                        $tabelaCSV .= ($valor?(is_numeric($valor)?number_format($valor, 0, ',', '.'):$valor):(!is_numeric($valor)?'-':$valor));
                                    }else{
